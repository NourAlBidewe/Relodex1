...
// TEST first, didnt test yet, updated to be used in the mean time
...
//  p is object (provider or user) to be added to database
  static add(var p) {
    String path = (p is User)
        ? "Users/961-${p.phone}"
        : "${p.prof_path.split("/")[0]}/961-${p.phone}-${p.prof_path.split("/")[1]}";
    Firestore.instance.document(path).setData(p.toJson());
  }
...
//   yet to edit, so that when called it'll return the next n service providers, for when user is scrolling down the list
  static getnServiceProviders(String category, String subCategory, int n, String sorting_criteria, bool descending, User us) async {
    Query cr = Firestore.instance.collection(category)
...
//  p is object (provider or user) to be deleted from database
  ...
  static updateServiceProviderStars(ServiceProvider sv, User us, int num_stars) async {
    String path = "${sv.prof_path.split("/")[0]}/961-${sv.phone}-${sv.prof_path.split("/")[1]}";
    ...
    dr2.get().then((out) {
      ...
      if (us.ratings["961-${sv.phone}"] != null)
        ...
        us.ratings.addAll({"961-${sv.phone}": {"badge": 0, "stars": num_stars}});
      dr2.updateData({"ratings": us.ratings});
    });
  }

  static previous(ServiceProvider sv, User us, String rate) {
    if (us.ratings["961-${sv.phone}-${sv.prof_path.split("/")[1]}"] == null)
      return 0;
    else
      return us.ratings["961-${sv.phone}-${sv.prof_path.split("/")[1]}"][rate];
  }

//  badge_index 1 or 2 or 3
//  ret: first time to give badge: 1
//       giving same badge: 0
//       giving different badge: -1
  static int incrementServiceProviderBadges(ServiceProvider sv, User us, int badge_index) {
    ...
    dr.get().then((out) {
      ...
      if (previous(sv, us, "badge") != 0)
        sv.badges[us.ratings["961-${sv.phone}-${sv.prof_path.split("/")[1]}"]["badge"] - 1] -= 1;
      sv.badges[badge_index - 1] += 1;
      dr.updateData({"badges": sv.badges});
    });
    if (previous(sv, us, "badge") != 0)
      ret = us.ratings["961-${sv.phone}-${sv.prof_path.split("/")[1]}"]["badge"] == badge_index ? 0 : -1;
    path = "Users/961-${us.phone}";
    DocumentReference dr2 = Firestore.instance.document(path);
    dr2.get().then((out) {
      us.ratings = out["ratings"];
      if (us.ratings["961-${sv.phone}-${sv.prof_path.split("/")[1]}"] != null)
        us.ratings["961-${sv.phone}-${sv.prof_path.split("/")[1]}"]["badge"] = badge_index;
      else
        us.ratings.addAll({"961-${sv.phone}-${sv.prof_path.split("/")[1]}": {"badge": badge_index, "stars": 0}});
      dr2.updateData({"ratings": us.ratings});
    });
    return ret;
  }
  ...
  static addFavorite(User us, ServiceProvider sv) {
    ...
    DocumentReference dr = Firestore.instance.document(path);
    dr.get().then((out) {
      us.favorites.addAll({"961-${sv.phone}-${sv.prof_path.split("/")[1]}": sv.prof_path});
  ...
  static removeFavorite(User us, ServiceProvider sv) {
    ...
    dr.get().then((out) {
      ...
      us.favorites.remove("961-${sv.phone}-${sv.prof_path.split("/")[1]}");
      dr.updateData({"favorites": us.favorites});
    });
  }

  static getOtherJobs(ServiceProvider sv) async {
    List<String> categories = ["نجّار", "ميكانيكي", "طبيب", "ديكور", "دهان و نش", "سمكري", "سمسار", "كهربجي", "عامل بناء", "فلّاح", "بلّاط", "تنظيف سجّاد", "مدرّس", "خيّاط", "مصبغة", "كندرجي", "صالون", "مصوّر", "حدّاد"];
    List<ServiceProvider> lst = List();
    for(String s in categories) {
      QuerySnapshot qs = await Firestore.instance.collection(s).where("phone", isEqualTo: sv.phone).getDocuments();
...
//    StorageReference sr = FirebaseStorage.instance.ref().child("hello");
    File("/assets/logo.PNG").readAsBytes().then((out) {
      print(out);
    });
  }
}
